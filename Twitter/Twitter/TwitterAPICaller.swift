...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func favorTweet(tweetID:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        ...
        TwitterAPICaller.client?.post(url, parameters: ["id":tweetID], progress: nil, success: {(task: URLSessionDataTask, responds:Any?) in success()}, failure:{(task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    }
